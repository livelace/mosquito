...
class Mosquito(object):
    ...
    def _handle_content(self, source_id, original_content, expanded_url, timestamp):
        ...
        config_timestamp = config_data[0][9]
        operation_timestamp = time.mktime(datetime.utcnow().timetuple())
        
        if timestamp > config_timestamp:
            ...
            if self.mail.active:       
                if not self.mail.send(destination, header, priority, subject, original_content, expanded_text_content, expanded_image_content):
                    self.db.add_archive(
            ...
            self.logger.debug('Content timestamp is lower than configuration timestamp')
                ...
                for data in config_data:
                    ...
                    if config_enabled == 'True':
                        if time.mktime(datetime.utcnow().timetuple())-config_timestamp > update_interval:
                            ...
                            if plugin == 'rss':
                                ...
                                for post in posts:
                                    ...
                                    if self._check_regexp(original_content, regexp_list):
                                        self._handle_content(source_id, original_content, 
                                                             expanded_url, post_timestamp)
                            ...
                            self.logger.debug('Update interval has not been reached for the configuration: {} -> {}'.format(plugin, source))
                    else:
