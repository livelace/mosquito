...
class Mosquito(object):
    ...
    def _handle_content(self, source_id, original_content, expanded_url):
        ...
        header_list.append('X-mosquito-expanded-url:' + self._convert_encoding(expanded_url))
                        ...
                        if args.force or (current_timestamp - config_timestamp) > update_interval:
                            ...
                            if count > 0:
                                ...
                                if current_timestamp > (config_timestamp + int(update_alert)):
                                    ...
                                    self.mail.send(destination_list, None, None, '***No new data from the configuration***', '{} -> {} -> {}'.format(source_id, plugin, source), None, None, None)
                        else:
